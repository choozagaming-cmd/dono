...
const activeProfile = new URLSearchParams(location.search).get('profile') || localStorage.getItem('streamnest:activeObsProfile:v1') || 'landscape-1080';
function profileLayoutKey(id){return `streamnest:overlayLayout:profile:${id}`;}
function loadProfileLayout(){try{const raw=localStorage.getItem(profileLayoutKey(activeProfile));if(raw)return JSON.parse(raw);}catch(_){}return layoutApi.load();}
function saveProfileLayout(value){try{localStorage.setItem(profileLayoutKey(activeProfile),JSON.stringify(value));localStorage.setItem('streamnest:activeObsProfile:v1',activeProfile);}catch(_){}}
let layout = loadProfileLayout();
...
function dataFor(event){const baseLabel=event.label||(event.type==='drop'?`${event.rarity||'RARE'} DROP`:event.type==='challenge'?'CHALLENGE UNLOCKED':event.tts?'TTS SUPPORT':'SUPPORT');return{label:event.demo?`DEMO • ${baseLabel}`:baseLabel,name:event.name||'Anonymous',amount:event.demo?'DEMO':money(event.amount),title:event.title||'',message:event.message?`“${event.message}”`:'',icon:event.icon||(event.type==='challenge'?'🎯':event.type==='drop'?'✨':'✦'),ttsBadge:event.tts?'🔊 TTS':'',dropMedia:event.assetUrl||'',media:event.assetUrl||''};}
...
function playSound(event,scene){const url=event.soundUrl||scene?.soundUrl;if(!url)return;try{const a=new Audio(url);a.volume=Number(event.soundVolume??scene?.soundVolume??.88);a.play().catch(()=>{});}catch(_){}}
...
function renderEvent(event,done){layout=event.__layout||loadProfileLayout();scaleRoot();const mapped=event.type==='drop'?'drop':event.type==='challenge'?'challenge':'support';const key=(event.sceneKey&&layout.scenes[event.sceneKey])?event.sceneKey:mapped;const sc=layout.scenes[key]||layout.scenes.support;root.innerHTML='';const nodes=[];sc.elements.slice().sort((a,b)=>a.z-b.z).forEach(el=>{const n=createEl(el,event);if(n){root.appendChild(n);nodes.push({n,el});}});root.classList.add('show');animateIn(nodes,sc);playSound(event,sc);const exit=()=>animateOut(nodes,()=>{root.classList.remove('show');root.innerHTML='';done();});const finish=()=>{currentTimer=setTimeout(exit,event.tts?1200:(sc.durationMs||5000));};if(event.tts&&event.message)speak(event,finish);else finish();}
...
if(window.StreamPreviewTransport){window.StreamPreviewTransport.listen((packet)=>{if(!packet)return;if(packet.profile&&packet.profile!==activeProfile)return;if(packet.kind==='studio-layout-sync'&&packet.layout){layout=packet.layout;saveProfileLayout(layout);scaleRoot();document.documentElement.dataset.layoutSync='saved';return;}if(packet.kind!=='studio-preview'||!packet.event)return;const event={...packet.event,verified:true,preview:true};if(packet.layout)event.__layout=packet.layout;enqueue(event);}).then((state)=>{if(state?.connected)document.documentElement.dataset.realtime='connected';}).catch(()=>{});}
